...
// sortGroupsByName mengurutkan grup berdasarkan nama (alfabet + angka)
func sortGroupsByName(groups []GroupInfo) []GroupInfo {
	...
	sort.Slice(sorted, func(i, j int) bool {
		...
		return compareNames(nameI, nameJ)
	})

	return sorted
}

// compareNames membandingkan dua nama dengan urutan: angka dulu, lalu alfabet
func compareNames(name1, name2 string) bool {
	// Normalize: ubah ke lowercase untuk perbandingan
	name1 = strings.ToLower(strings.TrimSpace(name1))
	name2 = strings.ToLower(strings.TrimSpace(name2))

	// Jika dimulai dengan angka, angka lebih dulu
	r1 := []rune(name1)
	r2 := []rune(name2)
	...
	isNum1 := unicode.IsDigit(r1[0])
	isNum2 := unicode.IsDigit(r2[0])
	...
	// Jika sama-sama angka atau sama-sama huruf, bandingkan secara lexicographic
	return name1 < name2
}
...
func showGroupMenu(telegramBot *tgbotapi.BotAPI, chatID int64, client *whatsmeow.Client) {
	menuMsg := `👥 **MANAJEMEN GRUP WHATSAPP**
...
📋 **Daftar Grup**
Lihat semua grup WhatsApp yang telah terdeteksi

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

💡 **Catatan:**
• Grup akan otomatis terdeteksi saat bot menerima pesan dari grup
• Gunakan tombol di bawah untuk melihat daftar grup`
	...
	keyboard := tgbotapi.NewInlineKeyboardMarkup(
		tgbotapi.NewInlineKeyboardRow(
			tgbotapi.NewInlineKeyboardButtonData("📋 Lihat Daftar Grup", "list_grup"),
		),
		tgbotapi.NewInlineKeyboardRow(
			tgbotapi.NewInlineKeyboardButtonData("🔙 Kembali ke Menu", "refresh"),
		),
	)
