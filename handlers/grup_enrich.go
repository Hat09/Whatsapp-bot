	...
	for batchStart := 0; batchStart < len(needsAPIGroups); batchStart += batchSize {
		...
		for idx := batchStart; idx < batchEnd; idx++ {
			...
			go func(groupIdx int) {
				...
				// Pastikan cancel dipanggil di akhir goroutine
				defer func() {
				...
				for retry := 0; retry < maxRetries; retry++ {
					...
					if retry < maxRetries-1 {
						// Cancel context lama sebelum membuat yang baru
						if currentCancel != nil {
							currentCancel()
						}
						...
						ctx, newCancel = context.WithTimeout(context.Background(), APICallTimeout)
						currentCancel = newCancel // Update untuk defer di akhir
					}
				}
				// currentCancel akan di-cancel oleh defer di atas
